...
IAB.lua reads `.schem` and `.litematic` itself, from `autoBuilder/converter`.
Pick one in the Schematic section on the Build tab and it converts there and
then, so Preview Build shows it without a save step — the same way a model
works. It uses the same table these scripts do — generated by
`export_blockmap.py` rather than typed twice — and produces the same result:
Mawglass Citadel converts to 386,197 blocks either way, same size, same block
counts, nothing unmapped.

It hollows too. Hidden Blocks drops only what is sealed inside, so the build
looks identical and costs less; Outside Only keeps the shell and throws away
everything behind it. Which is worth using depends entirely on the build — on
Mawglass Citadel the first finds 1.5% and the second 58.5% — so it is a choice
rather than a switch.

It also blends. A converted build often reads as scattered blocks because
Minecraft's stone, andesite, diorite and their polished cousins each map to a
different Islands block, so one wall arrives as five slightly different greys.
Blend to Colours keeps the most-used blocks and moves the rest onto the nearest
one that looks the same, judged by colours measured in game. Where a colour
sits between two kept blocks it alternates between them by position, so the
join reads as the shade in between rather than a hard edge.
